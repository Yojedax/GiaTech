  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Total procesadas', value: aud.total.toLocaleString(), color: 'var(--text-primary)' },
          { label: '✓ Alta confianza', value: `${aud.pctAlta}%`, color: '#166534' },
          { label: '~ Media confianza', value: `${aud.pctMedia}%`, color: '#d97706' },
      ...
      <div className="px-5 py-4 rounded-xl text-sm font-medium" style={{ background: 'rgba(0,212,255,0.05)', color: '#00d4ff', border: '1px solid rgba(0,212,255,0.12)' }}>
        💡 Impacto estimado: <strong>{aud.ahorroEstimado}</strong> — resultado de reducir entregas fallidas y mejorar la calidad del padrón.
      </div>

      {/* Search */}
      <div className="card p-5">
        <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Normalizador de domicilios</div>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} />
            <input type="text" value={inputValue} onChange={e => setInputValue(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && inputValue && handleSearch(inputValue)}
              placeholder="Ingresá una dirección, ej: Corrientes 1234, Buenos Aires"
      ...
      {loading && (
        <div className="card p-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
            ...
            Consultando padrón y geocodificando…
          </div>
        </div>
      )}
        ...
        <div className="card p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span style={{ color: 'var(--text-muted)' }}>Score de confianza</span>
          ...
          {result.problema && (
            <div className="flex items-start gap-2 p-3 rounded-xl text-xs" style={{ background: '#3d2d0a' }}>
              ...
              <div>
                <div className="font-semibold" style={{ color: '#d97706' }}>Observación:</div>
      ...
      {/* Quality bars */}
      <div className="card p-5">
        <div className="text-sm font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Distribución de calidad del padrón</div>
        <div className="space-y-3">
          {[
            { label: 'Alta confianza', value: aud.alta, pct: aud.pctAlta, color: '#166534' },
            { label: 'Media confianza', value: aud.media, pct: aud.pctMedia, color: '#d97706' },
